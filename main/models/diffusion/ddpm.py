...
from main.util import get_named_beta_schedule, extract
...
class DDPM(nn.Module):
    def __init__(
        ...
        self.beta_1 = beta_1
        self.beta_2 = beta_2
        self.beta_schedule_name = beta_schedule_name
        self.var_type = var_type
        self.pred_mode = decoder.pred_mode
        self.use_adaptive = decoder.use_adaptive

        # Main constants
        self.register_buffer("betas", torch.from_numpy(get_named_beta_schedule(self.beta_schedule_name, self.T)))
        ...
        # q(x_t-1 | x_t, eps) mean coefficients for adaptive_form1
        self.register_buffer(
            "post_coeff_3",
            self.betas / self.minus_sqrt_alpha_bar,
        )
        # # q(eps | x_t, x_0) coefficients for adaptive_form2
        self.register_buffer(
            "post_coeff_4",
    ...
    def get_posterior_mean_covariance(
        self, x_t, t, clip_denoised=True, cond=None, z=None, guidance_weight=0.0
    ):
        B = x_t.size(0)
        ...
        pred_score = self.decoder(x_t, t_, low_res=cond, z=z)
        # Compute updated score
        if self.use_adaptive:
            eps_score, x0_score, adaptive_factor = pred_score[..., :3, ...], pred_score[..., 3:6, ...], pred_score[..., 6:7, ...]
            if guidance_weight != 0:
                # todo update ddpm sampling from adaptive model with guidance
                ...
                # eps_no_cond_score = self.decoder(
                #     x_t,
                #     t_,
                #     low_res=torch.zeros_like(cond),
                #     z=torch.zeros_like(z) if z is not None else None,
                # )[..., :3, :, :]
                # eps_score = (1 + guidance_weight) * pred_score[..., :3, :, :] - guidance_weight * eps_no_cond_score
            else:
                # todo update ddpm sampling from adaptive model
                ...
        else:
            if guidance_weight == 0:
                eps_score = pred_score
            else:
                eps_score = (1 + guidance_weight) * pred_score - guidance_weight * self.decoder(
                    x_t,
                    t_,
                    low_res=torch.zeros_like(cond),
                    z=torch.zeros_like(z) if z is not None else None,
    ...
    def get_posterior_mean_from_noise(self, x_t, t_, _eps_score=None, cond=None, z=None):
        t_ = t_
        if _eps_score is None:
            eps_score = self.decoder(x_t, t_, low_res=cond, z=z)[..., :3, :, :] if self.use_adaptive else self.decoder(x_t, t_, low_res=cond, z=z)
        else:
    ...
    def get_posterior_mean_from_x0(self, x_t, t_, _x0_score=None, clip_denoised=True, cond=None, z=None):
        t_ = t_
        ...
        if clip_denoised:
            x0_score = torch.clamp(x0_score, -1.0, 1.0)
    ...
    def get_posterior_eps_from_xt_x0(self, x_t, t_, x_0=None, clip_denoised=True, cond=None, z=None):
        t_ = t_
        if x_0 is None:
            x0_score = self.decoder(x_t, t_, low_res=cond, z=z)[..., 3:6, :, :] if self.use_adaptive else self.decoder(x_t, t_, low_res=cond, z=z)
        else:
        ...
        if clip_denoised:
            x0_score = x0_score.clamp(-1.0, 1.0)

        pred_eps = (
            extract(self.post_coeff_5, t_, x_t.shape) * x_t -
            extract(self.post_coeff_4, t_, x_t.shape) * x0_score
        )
        return pred_eps
    ...
    def sample(
        ...
        for idx, t in enumerate(reversed(range(0, num_steps))):
            ...
            (
                ...
                x, t, cond=cond, z=z, guidance_weight=guidance_weight
            )
            nonzero_mask = (
            ...
            # Langevin step!
            x = post_mean + nonzero_mask * torch.exp(0.5 * post_log_variance) * noise
    ...
    def compute_noisy_input(self, x_start, eps, t_):
        assert eps.shape == x_start.shape
        # Samples the noisy input x_t ~ q(x_t|x_0) in the forward process
        return x_start * extract(self.sqrt_alpha_bar, t_, x_start.shape) + eps * extract(
            self.minus_sqrt_alpha_bar, t_, x_start.shape
        )

    def forward(self, x, eps, t, low_res=None, z=None):
        # Predict noise
        x_t = self.compute_noisy_input(x, eps, t)
        return self.decoder(x_t, t, low_res=low_res, z=z), x_t
